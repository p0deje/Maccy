...
| 后台 OCR | 02-IMG-005 | `VNRecognizeTextRequest.perform` 移出 `@MainActor`,放到后台队列/actor;item 删除时取消 | OCR 不阻塞主线程 |
| 真异步任务 | 02-IMG-004 | 去掉 `Task{@MainActor}`,改后台 actor + 回主线程赋值 | 真正并行 |

**验收**:复制大图/打开预览期间 UI 不卡;Instruments 主线程无解码/resize/OCR 长尾。
...
2. **搬离主线程**:解码/resize/OCR/搜索/去重/富文本解析全部进 actor(P1+P2+P3+P4)。
