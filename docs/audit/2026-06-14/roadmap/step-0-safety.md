...
- [ ] **0.3 `recoverContainer` 改隔离策略** — `Storage.swift`。失败时把现有 store 文件(`-sqlite/-shm/-wal`)移动到 `<dir>.corrupted-<stamp>/`(stamp 由调用方传入,避免在库内用 `Date.now`),新建空 store,并通过回调(新增 `init` 参数 `onCorruption: ((URL) -> Void)?`)通知 UI 弹确认;**不再无条件删除**。`preconditionFailure` 保留为最终兜底。
- [ ] **0.4 加恢复测试** — `MaccyTests/StorageRecoveryTests.swift`:注入坏 store → 断言文件被**移动**到 `.corrupted-*` 而非删除,且返回可用 in-memory/空 container。用可控时间戳参数避免依赖系统时钟。
