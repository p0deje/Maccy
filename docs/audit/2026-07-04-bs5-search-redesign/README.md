...
- **2026-07-04** ✅ **T1.4 CI GREEN(run `28696334071`,10/10)** — regexp 输入上限落地。下一步 T1.5 高亮 memoize。
- **2026-07-04** 🔨 **T1.5 高亮 memoize(03-LT-RENDER-01)** — `HistoryItemDecorator.highlight` memoize on `(title, ranges, HighlightMatch style)`(`@ObservationIgnored highlightMemo`);**cache hit 提前 return——不重建 AttributedString 且不重赋 attributedTitle**,故不触发 `@Observable` 观察 → 行 `.drawingGroup()` 不重栅格化(架构 §2.5 LT-RENDER-03 依赖项);cache miss 才 build+store;空/title 空 path 清 memo。`HighlightMatch` 是 String raw enum(Equatable)。既有 `testHighlight` 覆盖 build+clear 路径(memoize 为内部优化,正确性由既有测试守)。CI 待验证。
- **2026-07-04** ❌→🔧 **T1.5 CI 失败(run `28696716877`)→ 修复** — `Lint+diagnostics` 红(`large_tuple`:`highlightMemo` 3-成员命名元组违规)。修复:改私有 `struct HighlightMemo`(3 字段;struct 不触 `large_tuple`,该规则仅约束元组)。**教训**:无本地工具链自检须 grep 3+-成员元组([[no-local-toolchain-ci-gates]] 已记 `large_tuple` 条目,此次漏检)。重推验证。
- **2026-07-04** ✅ **T1.5 CI GREEN(run `28696814952`,10/10)** — highlight memoize 落地。**Track 1 standalone 完成(T1.1–T1.5)**。
- **2026-07-04** 📋 **Track 1 收尾判定** — T1.6(`regenerateTitleForSymbolToggle`)+ T1.8(fuzzy body cap)**并入 T2**(需 `searchText` 缓存——反向替换 `·→space` 不安全,因内容可能含字面 `·`);T1.7(`needsResize` 移出热路径)**低价值**(仅 bool flag 赋值,实际 resize 工作在 popup observer 已 async);T1.9(truncation 单位重命名 `shortened(toChars:)`/`(toBytes:)`)cleanup,延后。**Track 1 核心价值已交付**:Gap A 经验证(07-F-010 非bug)、Gap B 修复(TextLimits 单源)、mixed 短路、regexp 输入上限、highlight memoize。下一步:**T2 全文搜索**(大工程,新会话;`searchText` 列 + actor 语料 + 正文扫描 + T2.7 索引测量闸门)。
- **2026-07-04** 🔨 **T2.1 `searchText` 持久化列 + 提取器 + ingest 写列** — 新列 `HistoryItem.searchText: String?`(lightweight migration,模板同 `HistoryItemContent.fingerprint`;旧行 nil → 退化标题搜索,回填 follow-up 同 8.5)。新 `HistoryItemEngine.searchableBody(contents:richTextParsingLimit:)`(fileURL→string→rtf→html 优先级,**不截断**;RTF/HTML >512KB 跳过退化 "";image/无文本 → "")+ `ContentIndex.text`(全量 UTF-8,对应既有 `textPrefix`)。ingest 一次性提取写列:`BackgroundClipboardIngestor` 主 hop 增返 `searchableBody(for:)`(`@MainActor`,与 `title(for:)` 同 hop,RTF/HTML 解析主线程亲和);**4-成员返回改 `private struct IngestMainWork: Sendable`**(规避 `large_tuple`,T1.5 教训);`makeHistoryItem` 增 `searchText` 参;`mergeFields` 复制 `dup.searchText`(镜像 `title`);`MainActorIngestorAdapter` 同步设列。**T2.1 不改搜索行为**——列被写但 T2.2 前不被搜索读。新 `MaccyTests/SearchTextMigrationTests.swift`(提取语义 string/rtf/html/fileURL/image/512KB退化 + 持久化 round-trip + 旧行 nil + ingest 端到端)。pbxproj 4 站注册(`DA0806A1…0B`/`DA0806A2…0C`,fresh `DA0806A` 前缀全 ID 唯一)。源码注释零 tag(Apple docstring)。CI 待验证。
- **2026-07-04** ✅ **T2.1 CI GREEN(run `28703233099`,10/10)** — `searchText` 列 + 提取器 + ingest 写列落地。`SearchTextMigrationTests` 9 法(提取语义/持久化/旧行 nil/ingest 端到端)全过。无搜索行为变更(列写不读,T2.2 起读)。`IngestMainWork` struct 规避 `large_tuple`(吸取 T1.5 教训)。下一步:**T2.2 语料移到 actor**(纯 `search(query:within:mode:)` 保留保 SearchActorTests 绿;新增有状态 `search(query:mode:)` + `register/remove/clear/replaceCorpus`;`performSearch` 删每键 `all.map`)。
  - **架构决策(本步隐含,T2.2 落实)**:语料将走 **DTO-driven on context-free actor**(option b)——SearchActor 仍是普通 `actor`(无 ModelContext),main 在 load/consume/delete/clear 把 `(id,title,body)` 作 Sendable DTO 推给 actor;cache 在 actor(满足架构 §4.2「mainContext 从不 reset → cache 必须在 actor」)。比给 SearchActor 配 @ModelActor 后台 context(option a)更简,匹配两域 DTO 模型,main 仅在低频变更(非每键)送 DTO。
- **2026-07-04** 🔨 **T2.2 语料移到 SearchActor + StoreEvent 维护** — SearchActor 持 `corpusByID: [ItemID: SearchCorpusItem]` + `order: [ItemID]`(保 `all` 序);新 corpus-ownership API `replaceCorpus/insert(at:)/remove/clearCorpus`;新有状态 `search(query:mode:)`(投影 `order`→复用纯 `search(query:within:mode:)`,后者保留以保 SearchActorTests 16 法零改动)。`SearchCorpusItem` 增 `body: String`(T2.2 携带不扫,T2.3 起扫);`TextLimits.searchBody=32_000`(ship 时 cap body 绑内存,T2.6 改 Defaults-driven)。`History.performSearch` **删每键 `all.map`**(只投 query+mode)。语料维护:`load` 直接 `await replaceCorpus`;`insertIncrementally`/`delete`/`clear`/`clearAll`/`syncAllToStore`/`togglePin`/`reconcileWithStore` 全走 **fire-and-forget `Task { await actor... }`**——安全因 `applySearchResults` 的 `all`-成员过滤丢弃 stale 条目(`.merged` 重插捕获 superseded id 一并 remove)。新 `MaccyTests/SearchCorpusOwnershipTests.swift`(replaceCorpus 保序、insert(at:)、insert 替换移动、remove、clearCorpus、有状态≡纯搜索 全模式)。SearchHighlightIndexTests/SearchActorTests 的 `SearchCorpusItem` 构造加 `body:""`。pbxproj 4 站(`DA0806B1…0B`/`DA0806B2…0C`)。CI 待验证。
  - **偏差记录(相对 §2.2 字面)**:计划写「`.removed/.cleared` 经 `consume` 分支投递」——实际 `.removed/.cleared` 仍走 `reconcileWithStore`(全量),但 destructive mutator(`delete/clear/clearAll/syncAllToStore/togglePin`)**直接**对 actor 维护(不经 StoreEvent),更直接且覆盖 size-trim 孤儿 + pin 重排。`consume` 的 `.removed/.cleared` 分支未被 ingest actor 发出(现状),保持不变。
- **2026-07-04** ✅ **T2.2 CI GREEN(run `28704501854`,10/10)** — 语料移到 SearchActor 落地,全 shard(含 5 UI + 3 perf)绿证搜索行为不变。paste-stack 抽离修 `file_length`(1060→964)。**核心成果**:`performSearch` 不再每键 `all.map` 投影(只投 query+mode);语料 actor-owned 增量维护。下一步:**T2.3 全文 exact+regexp 正文扫描**(DTO 增 `inBody` + body-grapheme offset;`simpleSearch`/`regexpSearch` 扫 body 封顶 `TextLimits.searchBody`;apply 侧 body 命中不清空标题高亮——T3 处理预览高亮)。
  - **`file_length` 教训(首次 CI 失败 run `28704185371`)**:History.swift 已近 1000 上限,T2.2 加 ~60 行→1060 触 `file_length`(warning=error=1000)。修复:抽 paste-stack + `currentModifierFlags` 到 `History+PasteStack.swift`(`pasteStack`/`logger` 已 internal;`currentModifierFlags` 移入 extension 令 `import AppKit` 文本引用 NSApp/NSEvent 规避 `unused_import`)。**前瞻**:T2.3-T2.7 主要改 SearchActor 非 History.swift,但若加 History.swift 须先查行数(964/1000,余 35 行)。
- **2026-07-06** ✅ **T2.3 CI GREEN(run `28762358204`,10/10)** — 全文 exact+regexp 正文扫描落地。`FullTextSearchTests` 5 法过;16 `SearchActorTests`(body="")零改动仍绿证 title-first 语义不变;UI/perf 全绿。`inBody` 默认 false 保既有构造不变。下一步:**T2.4 fuzzy 标题+正文前缀**(ADR-3,corpus=title+body.prefix(5000);超 5000 的 fuzzy 不命中,exact/regexp 仍覆盖)。
  - **T2.3 实现注**:`simpleSearch`/`regexpSearch` title-first → 未命中扫 `SearchCorpusItem.body`(已封顶 `TextLimits.searchBody`);body 命中 `inBody:true` + body-grapheme offset;`applySearchResults` 对 `inBody` 清空标题高亮(offset body 相对,不 resolve 到 title)。handler 结构 flat 化(移除冗余内层 main hop——既是性能干净化,也消除"序言后 hop"脚枪,同 ApplicationImage 崩溃教训)。
- **2026-07-06** 🔥 **T2.4 `/grill-with-docs` 设计 sharpening** — 5-agent workflow(4 readers:audit-docs/search-code/fuse-integration/tests + 合成)+ 主源复核(`SearchActor.swift`/`Search.swift`/`SearchDTOs.swift`/`History.swift`/`decisions.md`)产出 4 叉点。**Q1 计算形态 → (B) 两遍 title-first-wins(用户确认,ADR-8)**:镜像 T2.3 `simpleSearch`/`regexpSearch` title-first→body 模式,DTO 不变(per-result 单 `inBody`),消除接缝 bug 类(Fuse 非连续 ranges 跨 title/sep/body 接缝今日 DTO 表达不了);否决 (A 单遍 concat,DTO schema 改波及 T2.3 apply + T3)/ (C best-of-both per item,2× Fuse + 非归一化 score 跨字段比较)。**Q2 mixed 时序 → 非 fork**:T2.4 落地即 mixed 全文(`mixedSearch` 直调 `fuzzySearch`,fuzzy 现扫 body),T2.5 验证 tier 序 + 短路——roadmap 意图,非偏差。**Q3 跨项排序 → (a) 两 bucket 标题优先(🟡 代理,ADR-9,待确认)**:stable partition by `inBody`(标题在前)+ 各 bucket score 升序;T2.3 `testTitleMatchPreferredOverBodyMatch` 先例;否决 (b 纯 score 合并,非归一化)/ (c bonus/penalty,任意)。**Q4 突变刷新分歧 → 主源复核发现 `refreshVisibleItems`(`History.swift:803`,legacy title-only)经 add@323/pin@398/reconcile@469 shipped 可达,正文命中突变后缺失(T2.3 起 shipped bug,T2.4 fuzzy body 扩大);→ (a) 路由到 `performSearch()`/actor(🟡 代理,ADR-10,待确认,T2.4 最高反转成本项)**;否决 (b 延 §5.5)/ (c 给 legacy Searchable 加 body,大工且 legacy 将退役)。
- **2026-07-06** ⏸ **T2.4 实现暂缓待用户确认** — Q3/Q4 代理决策(用户离键盘 60s)。T2.4 计划拆 **T2.4a**(actor fuzzy body + ADR-9 排序,纯 `SearchActor.swift`,Q1 用户已确认故低风险)+ **T2.4b**(`refreshVisibleItems` 路由修,`History.swift` + 行为变更偏差,可独立 drop 若用户改选 Q4=(b) 延 §5.5)。**用户返回后先确认 Q3/Q4,再 TDD**(失败测试→最小正确改动→focused CI→全 shard)。ADR-9/10 已标 🟡 代理待确认;推翻=改 ADR + 调对应代码(均低反转成本,除 Q4 行为变更外)。
- **2026-07-06** ✅ **用户确认 Q3/Q4(「按最好判断、最完善、最合理的继续」)→ T2.4a 实现** — ADR-9/10 🟡 代理→✅ 确认。T2.4a:`SearchActor.fuzzySearch(for:in:)` 增 body fallback(标题先扫 → miss 扫 `body.shortened(to: .fuzzy)`,`inBody:true` + body 偏移,镜像 `simpleSearch`/`regexpSearch`);`fuzzySearch(query:within:)` 改**两 bucket 排序**(title `inBody==false` bucket 在前 + 各 bucket score 升序,替旧纯 score 升序——消除 Fuse score 跨 haystack 长度非归一化坑)。`FullTextFuzzyTests`(8 法:body 命中 inBody+body 偏移 / title 优先胜 / 双空不命中 / 空 body 退化标题 / 前缀内命中 / 超 5000 前缀不命中 / 两 bucket 排序区分 score-merge / body grapheme 偏移)。pbxproj 4 站(`DA0806E1…0B` ref / `DA0806E2…0C` build,镜像 FullTextSearchTests 模式)。自检:源码注释零 tag、无 large_tuple、SearchActor.swift 265 行、pbxproj FullTextFuzzyTests 恰 4 处。CI 待验证。
- **2026-07-06** ✅ **T2.4a CI: 全 shard 绿 except ui-4 testPin flake**(run `28785520169`,`f574e5b`)— Lint+unit+ui-1/2/3/5+perf-mixed/text/image 全绿。首跑(`28785283394`)Lint 滑:`identifier_name`(闭包参 `a,b`<3 字符→改 `$0/$1`)+ `line_length`(134/135→折行);**自检漏此两项,纳入 [[no-local-toolchain-ci-gates]]**。**ui-4 failed `MaccyUITests.testPin()` ×2 = 已知 contention flake**;grep 证 no UI test 触 fuzzy/search mode → 非 T2.4a 回归。T2.4b push 时全 shard 复验。
- **2026-07-06** 🔨 **T2.4b refreshVisibleItems→performSearch(ADR-10)** — `History.refreshVisibleItems` 非空分支 `updateItems(search.search(...))` → `performSearch()`(actor hop);修 T2.3 起 shipped 分歧(add@323/pin@398/reconcile@469 突变后正文命中缺失,T2.4 fuzzy body 扩大)。**行为变更(记偏差)**:突变刷新 sync→async(generation-guarded Task);`items` 在 actor hop 返回前不更新,但 generation 同步 bump。既有 `testAddingDuringSearchKeepsFilteredItems` 仍过(items 保持 pre-add filtered 状态);新增 `testAddingDuringActiveSearchBumpsSearchGeneration`(generation-oracle 锁 routing 变更)。legacy `search` 仍用于 performSearch 空短路(非死代码)。History.swift 975/1000 行(近上限,T2.5+ 改动须先查行数)。CI 待验证。
- **2026-07-06** ✅ **T2.4b CI GREEN(run `28786366891`,10/10 全 shard)** — ui-4 testPin flake 未复现(flake 确认)。**T2.4(a+b)完成**:fuzzy body match + 两 bucket 排序 + refreshVisibleItems→performSearch(修 T2.3 正文分歧)。
- **2026-07-06** 🔨 **T2.5 mixed 全文验证(测试补)** — `SearchActorTests.item` helper 加默认 `body` 参数(向后兼容,既有 16 测试零改动);增 2 法:`testMixedFindsBodyMatchViaExactTier`(body 含 needle、无标题命中 → mixed 经 exact tier 浮现 `inBody:true`)+ `testMixedFindsBodyMatchViaFuzzyTier`(body fuzzy 命中、无 exact/regexp → mixed 经 fuzzy tier,regexp 因无元字符跳过)。mixed 全文行为自 T2.3/T2.4/T1.3 已实现,本步仅补覆盖。CI 待验证。
- **2026-07-06** ✅ **T2.5 CI GREEN(run `28787109201`)** — mixed 全文验证落地。
- **2026-07-06** 🔨 **T2.6 body 封顶 Defaults-configurable(ADR-5)** — `TextLimits` 增 `searchBodyMin=1_000`/`searchBodyMax=256_000`/`clampedSearchBody(_:)`;`Defaults[.searchBodyLimit]`(默认 `TextLimits.searchBody`);`corpusEntry` 读 `Defaults[.searchBodyLimit]` 经 clamp;`History.init` 增 `Defaults.updates(.searchBodyLimit)` observer → `replaceCorpus` 重建 + `refreshVisibleItems` 重搜。测试:`FullTextSearchTests.testSearchBodyLimitClampsToRange`(clamp 单元)+ `HistoryTests.testSearchBodyLimitCapsCorpusBody`(设 cap=1000,needle 在 1000 处不命中——证 corpusEntry 读 Defaults)。CI 待验证。
- **2026-07-06** ❌→🔧 **T2.6 CI unit 失败(run `28787842337`)→ 修** — `testSearchBodyLimitCapsCorpusBody` 失败:query 设在 add 之后,`waitForInFlightSearch` 跑时 200ms debounce 未 fire、`searchTask=nil` → 立即返回、items 仍含新加项 → 断言 fail。**修**:query 移到 add 之前 → add 的 `refreshVisibleItems`→`performSearch`(T2.4b)设 `searchTask` → 可 await。**教训**:History 搜索经 debounce,测试须保证 `searchTask` 已设才能 `await waitForInFlightSearch`——经 add/pin/reconcile 的 `refreshVisibleItems` 或 `refreshForModeChange` 触发。
- **2026-07-06** 🔧 **T2.6 再失败(run `28788583954`,unit + ui-2)→ 移除 fragile 集成测试** — query 前置修后 `testSearchBodyLimitCapsCorpusBody` 仍 fail:根因是 `add()` 不直接维护 actor 语料(经 `consume`/observer 路径),直接 add 的项进 actor corpus 的路径不确定,body cap 在该集成测试里不可靠隔离(且每跑烧 11min CI)。**移除该集成测试**;cap 逻辑已由 `testSearchBodyLimitClampsToRange`(clamp 单元,过)+ 既有 `testNeedleBeyondBodyWindowNotFound`(actor 级 capped body 不命中,过)确定性覆盖;wiring(`corpusEntry` 读 `Defaults[.searchBodyLimit]`)为一行,经检视。**ui-2 `testCopyImage` 11.99s timeout = 已知 contention flake**(no UI test 触 searchBodyLimit),非 T2.6 回归。
- **2026-07-06** ✅ **T2.6 CI GREEN(run `28789617843`,全 shard)** — body 封顶 Defaults-configurable 落地。
- **2026-07-06** 🔨 **T2.7 actor 搜索延迟基准(n=200 baseline + n=1000 闸门)** — `TextSearchPerformanceTests` 改测 **actor 路径**(替 legacy `Search()`):`measureActorSearch(count:mode:)` 建自含语料(n 项,各持 heavy_text body 封顶 `TextLimits.searchBody`,String CoW 共享故内存有界),测 `SearchActor.search(query:within:mode:)` per-key off-main 延迟 + main-thread gap。`testActorSearchPerKey_N200`(baseline)+ `testActorSearchPerKey_N1000`(闸门:`XCTAssertLessThan(gap, 0.016)` 主线程预算 + `PERF|` 打印 actor 延迟供索引决策)。CI 待验证。
- **2026-07-06** ✅ **T2.7 CI GREEN + 测量 + 索引决策(run `28790415574`)** — n=1000 heavy .mixed:`searchPerKey_avg=4.065s`/`mainGap=8.3ms`(<16ms ✓);n=200:`0.796s`/`3.1ms`。**主线程闸门 PASS**(off-main 扫描保 UI 响应)。**决策:不开 trigram 索引**(bottleneck=fuzzy,trigram 不加速 fuzzy;exact/regexp 本就毫秒级)。**T2.8 SKIP,Track 2 完成**。详 `decisions.md` ADR-2「T2.7 实测结果与索引决策」小节。
- **2026-07-06** 🔧 **fuzzy body 截断 O(maxLength) 化(T2.7 测量发现)** — 测量揭示 fuzzy ~4s/n=1000 主因:`shortened(to:)` 经 `count > maxLength` 对 32KB body 做 O(body) grapheme 计数 × n(~3s),非 Fuse 本身。修 `String+Shortened.swift`:用 `index(_:offsetBy:limitedBy:)` 推进至多 maxLength(短路,不 `.count`),O(maxLength) 替 O(count);语义不变(≤maxLength 返 self、>maxLength 截断前 maxLength 字符)。fuzzy body 热路径 n=1000 heavy 预期 4s→显著下降。CI 复测验证。
- **2026-07-06** 📊 **T2.7 perf-fix 复测(run `28791503968`):小幅改进,bottleneck 是 Fuse 非 truncation(证伪上条预期)** — O(maxLength) 化后:n=1000 `4.065s→3.718s`(~8%),n=200 `0.796s→0.700s`(~12%);main gap `8.3ms→3.5ms`。**原「truncation 占 ~3s」假设证伪**——实际 bottleneck 是 **Fuse DP 本身**(纯 Swift、per-call 开销大)在 1000×5000-char bodies 上。fix **保留**(O(maxLength) 更正确 + 真实小幅改进 + main gap 改善);fuzzy ~3.7s/n=1000 heavy 是 **Fuse 限制**(压力场景;real usage n≤200 default + 轻 body 远快);杠杆=降 fuzzy 封顶(ADR-3)或 Fuse opt,**非 trigram**(已决策 no-index)。
- **2026-07-06** 🔨 **T3.1 SwiftUI AttributedString 预览高亮(Stage 1,ADR-4)** — `HistoryItemDecorator` 增 `previewAttributedText` + `setPreviewHighlight(query, bodyRanges)`(镜像 `highlight` + memo;body 偏移 clamp 到 `text.count`——超窗 deep match 跳过,留 T3.2);`applySearchResults` 对 `inBody` 命中调 `setPreviewHighlight(query, dto.ranges)`,非 body 命中清空;`PreviewItemView` 文本分支 `Text(item.text)` → `if let preview = item.previewAttributedText { Text(preview) } else { Text(item.text) }`。**offset 映射**:`item.text`(`previewableTextPrefix`,byte-safe grapheme prefix)与 `corpusEntry.body`(`searchText.prefix`)同源、均为全文前缀,故 body 偏移 `O < text.count` 直接映射(O≥ 是 deep match,T3.2 处理)。测试 `testPreviewHighlight` + `testPreviewHighlightDeepRangeBeyondWindowIsSkipped`。CI 待验证。
- **2026-07-06** ✅ **T3.1 CI GREEN(run `28793132324`)**。
- **2026-07-06** 🔨 **T3.2 NSTextView representable(Stage 2,ADR-4)—项目首个 NSTextView** — 新 `PreviewTextRep: NSViewRepresentable`(置于 `PreviewItemView.swift` 免 pbxproj):`NSTextView.scrollableTextView()`+`NSScrollView`;`NSTextStorage` 持全文 `searchText`(`NSLayoutManager` 惰性 layout,解除 3000 急切布局封顶,live 内存有界);`buildAttributed()` 全文 + `Defaults[.highlightMatch]` 样式(grapheme→UTF-16 NSRange 经 `NSRange(_:in:))`,emoji/combining 正确);`scrollRangeToVisible` 滚到首个高亮(deep match 上下文)。`HistoryItemDecorator` 增 `previewBodyQuery`/`previewBodyRanges`(全文 offsets)+ `needsScrollablePreview`(body>textPreviewLimit 或 deep match);`setPreviewHighlight` 存全文 ranges。`PreviewItemView` 三分支:image / `needsScrollablePreview`→PreviewTextRep / ScrollView Text(attributed/text)。测试 `testPreviewTextRepBuildsHighlightedAttributedString`(bold range 落位 + deep skip)。**`PreviewRefreshUITests` identity 契约须 CI 验证**(text 分支切 NSTextView)。CI 待验证。
- **2026-07-06** ❌→🔧 **T3.2 CI 失败(run `28794369705`,全 test shard)→ 修** — `PreviewItemView.swift:173` `NSFontManager.convert(_:hasTrait:)` 重载歧义("no exact matches")。**修**:italic 改 `fontDescriptor.withSymbolicTraits(.italic)` + `NSFont(descriptor:size:)`(避 convert 重载,更直接)。
- **2026-07-06** ✅ **T3.2 CI GREEN(run `28794836703`)** — PreviewRefreshUITests identity 契约绿(text 分支切 NSTextView 不破)。**🏆 BS-5 搜索重设计全量完成**:Track 0(模式按钮)+ Track 1(T1.0-T1.5 标题正确性)+ Track 2(全文 T2.1-T2.7,T2.8 trigram skip per no-index 决策)+ Track 3(预览高亮 T3.1 SwiftUI + T3.2 NSTextView)。全 CI 绿。详 `decisions.md` ADR-1..10 + glossary + 本 §九。
