...
- **2026-07-04** ❌→🔧 **T2.2 CI 失败(run `28704185371`)→ 修复** — `Lint+diagnostics` 红(`file_length`:History.swift 1060 > 1000 限)。**根因**:T2.2 加 ~60 行推过限。**修复**:抽 paste-stack + `currentModifierFlags` 到新 `Maccy/Observables/History+PasteStack.swift`(`extension History`),History.swift 1060→964。选 paste-stack 因最自包含(`pasteStack`/`logger` 已 internal,仅 `currentModifierFlags` 跨文件需 internal——且移入 extension 令 `import AppKit` 被 NSApp/NSEvent 文本引用,规避 `unused_import`)。pbxproj 4 站(`DA0806C1…0B`/`DA0806C2…0C`)。**教训**:`file_length` warning=error=1000,History.swift 已近上限,T2.x 再加须先看行数([[no-local-toolchain-ci-gates]] 增 `file_length` 条目)。重推验证。
