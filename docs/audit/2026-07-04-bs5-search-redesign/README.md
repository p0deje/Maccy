...
- **2026-07-06** 🔨 **T2.3 全文 exact+regexp 正文扫描** — `SearchMatchDTO` 增 `inBody: Bool`(自定义 init 默认 `false`,保 16 个 SearchActorTests 的 `SearchMatchDTO(...)` 构造零改动);`SearchActor.simpleSearch`/`regexpSearch` 改 **title-first → 未命中扫 body**(body=已封顶的 `SearchCorpusItem.body`);body 命中 `inBody:true` + body-grapheme offset(`body.distance`)。`applySearchResults` 对 `inBody` 命中**清空标题高亮**(offset 是 body 相对,不能 resolve 到 title;item 仍在结果内;预览高亮留给 T3)。fuzzy 仍 title-only(T2.4)。mixed 经 simple/regexp 继承正文扫描(T2.5 精化)。新 `MaccyTests/FullTextSearchTests.swift`(exact/regexp deep body 命中 offset、title-preferred、query 两处皆无、超 body 窗不命中)。pbxproj 4 站(`DA0806D1…0B`/`DA0806D2…0C`)。History.swift 964→970(仍 <1000)。CI 待验证(push 自动触发)。
